...
const createUser = async(user) => {
    try {
      const query = 'INSERT INTO users (name, email, age, adress, cp, phone) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;';
      const response = await db.query(query, [user.name, user.email, user.age, user.adress, user.cp, user.phone]);
